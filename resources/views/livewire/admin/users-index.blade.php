<div class="space-y-6">
    <div class="flex items-center justify-between gap-4">
        ...
        <a href="{{ route('admin.users.create') }}" wire:navigate class="rounded-full bg-zinc-950 px-4 py-2 text-sm font-medium text-white dark:bg-white dark:text-zinc-950">Create User</a>
    </div>
                ...
                <tbody class="divide-y divide-zinc-200 dark:divide-white/10">
                    @foreach ($users as $managedUser)
                        <tr>
                            ...
                            <td class="px-6 py-4 align-top"><span class="rounded-full px-3 py-1 text-xs font-semibold {{ $managedUser->is_active ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-400/10 dark:text-emerald-200' : 'bg-rose-100 text-rose-700 dark:bg-rose-400/10 dark:text-rose-200' }}">{{ $managedUser->is_active ? 'Active' : 'Inactive' }}</span></td>
                            <td class="px-6 py-4 align-top text-sm text-zinc-600 dark:text-zinc-300">@if ($managedUser->landlordProfile) Landlord profile @elseif ($managedUser->caretakerProfile) Caretaker profile @elseif ($managedUser->tenantProfile) Tenant profile @else No dedicated profile @endif</td>
                            <td class="px-6 py-4 align-top text-sm text-zinc-600 dark:text-zinc-300">{{ $managedUser->last_login_at?->diffForHumans() ?? 'Never' }}</td>
                            <td class="px-6 py-4 align-top text-right"><a href="{{ route('admin.users.edit', $managedUser) }}" wire:navigate class="text-sm font-medium text-cyan-700 dark:text-cyan-300">Manage</a></td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
        </div>
        <div class="border-t border-zinc-200 px-6 py-4 dark:border-white/10">{{ $users->links() }}</div>
    </div>
</div>
